...
public class DAOClass 
{
	...
	public boolean insertData(StudentPersonalInfo ob)
	{
		...
		try
		{
			...
			{
			//create query
			pst = con.prepareStatement("Insert into StudentPersonalInfo values(?,?,?,?,?,?,?,?)");
	...
	public boolean deleteData(StudentPersonalInfo ob)
	{
		...
		try
		{
			...
			pst = con.prepareStatement("delete from StudentPersonalInfo where Stname =?");
			pst.setString(1, ob.getStname());
	...
	public ArrayList<StudentPersonalInfo> fetchAllRecord()
	{
		...
		{
			con = DBConnect.getMySQLConnection();
			pst = con.prepareStatement("select * from StudentPersonalInfo");
