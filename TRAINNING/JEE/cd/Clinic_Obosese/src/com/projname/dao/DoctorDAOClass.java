...
public class DoctorDAOClass 
{
	...
	public boolean deleteData(DoctorInfo ob)
	{
		...
		try
		{
			...
			if(con!=null)
			{
				pst = con.prepareStatement("delete from DoctorInfo where Demail =?");
	...
	public boolean updateData(DoctorInfo ob)
	{
		boolean f= false;
		Connection con =null;
		PreparedStatement pst =null;
		...
		{
			con =DBConnect.getMySQLConnection();
			pst = con.prepareStatement("update doctorinfo set Dmob= ?,Dqualification= ?,Dspecialisation= ?,Dexperience= ?,Dfees= ?,Dtime= ?,Dnoofpatient= ?,Dpass= ? where Demail = ?");
		//	
			pst.setString(1, ob.getDmob());
			pst.setString(2, ob.getQual());
			pst.setString(3, ob.getDspec());
			pst.setString(4, ob.getExp());
			pst.setString(5, ob.getDfees());
			pst.setString(6, ob.getDtime());
			pst.setString(7, ob.getDpno());
			pst.setString(8, ob.getDpass());
			pst.setString(9, ob.getDemail());
	...
	public int findFees(PatientAppointment pobj)
	{
		int fees;
		Connection con =null;
		PreparedStatement pst =null;
		...
		try
		{
			...
			if(con!=null)
			{
				pst = con.prepareStatement("select * from DoctorInfo where Demail =?");
				//pst.setString(1, uname);
				pst.setString(1, pobj.getDemail());
				rs=pst.executeQuery();
				if(rs.next())
				{
				   // System.out.println("FOUNND");
		...
		//System.out.println(ob.getDfees());
		fees = Integer.parseInt(ob.getDfees());
		return fees;
	}
	
	
	public int findNumber(String Demail)
	{
		int number;
		DoctorInfo ob =new DoctorInfo();
		Connection con =null;
		...
		number = Integer.parseInt(ob.getDpno());
		return number;
	}
